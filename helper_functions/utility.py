...
def check_password_admin():  
    ...
    def password_entered():  
        """Checks whether a password entered by the user is correct."""  
        if hmac.compare_digest(st.session_state["admin"], st.secrets["admin"]):  
            st.session_state["admin_password_correct"] = True  
            del st.session_state["admin"]  # Don't store the password.  
        else:  
    ...
    # Show input for password.  
    st.text_input( 
        "Admin Password", type="password", on_change=password_entered, key="admin"          
        )  
